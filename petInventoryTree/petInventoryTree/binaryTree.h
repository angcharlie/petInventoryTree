...
{
	root = nullptr;  //initializes binary tree to an empty state
}
...
{
	return (root == nullptr);	//empty if root is null. Returns true if root is equal to null 
}
...
//Private helper funciton for treeHeight 
template<class elemType>
...
//Private helper funciton for height 
template <class elemType>
...
//Private helper function for treeNodeCount 
template <class elemType>
...
//Helper function for treeLeavesCount 
template <class elemType>
int binaryTreeType<elemType>::leavesCount(nodeType<elemType> *p) const
{
	...
	else if (p->lLink == nullptr && p->rLink == nullptr)	//if a leaf node is reached 
	{
		return 1;
	}
	else //keep recursing 
...
//Private helpter function for destroyTree 
template <class elemType>
